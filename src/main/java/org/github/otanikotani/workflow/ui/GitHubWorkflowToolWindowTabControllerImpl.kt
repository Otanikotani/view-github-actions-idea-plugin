...
internal class GitHubWorkflowToolWindowTabControllerImpl(
    ...
    override var initialView = GitHubWorkflowToolWindowViewType.LIST
    override val componentController: GitHubWorkflowToolWindowTabComponentController?
        get() {
            for (component in mainPanel.components) {
                val controller = UIUtil.getClientProperty(component, GitHubWorkflowToolWindowTabComponentController.KEY)
        ...
        fun update() {
            ...
            guessAndSetRepoAndAccount()?.let { (repo, account) ->
                try {
                    ...
                    showWorkflowsComponent(repo, account, requestExecutor, wasReset)
                } catch (e: Exception) {
                    null
                }
            }
    ...
    private fun showWorkflowsComponent(
        ...
        val panel = GHLoadingPanelFactory(
            ...
            ComponentController(result, wrapper, disposable).also {
                UIUtil.putClientProperty(parent, GitHubWorkflowToolWindowTabComponentController.KEY, it)
            }
            wrapper
        }
    ...
    override fun canResetRemoteOrAccount(): Boolean {
        ...
        val matchingAccounts =
            authManager.getAccounts().filter { it.server.equals(singleRepo.ghRepositoryCoordinates.serverPath, true) }
    ...
    private inner class ComponentController(
        private val dataContext: GitHubWorkflowRunDataContext,
        private val wrapper: Wrapper,
        private val parentDisposable: Disposable,
    ) : GitHubWorkflowToolWindowTabComponentController {
        ...
        private val createComponentHolder = ClearableLazyValue.create {
            GHPRCreateComponentHolder(
                ActionManager.getInstance(), project, projectSettings, repositoryManager, dataContext, this,
                parentDisposable)
        }
